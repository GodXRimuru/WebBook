...
// ========== CREATE/UPDATE USER DOCUMENT FUNCTION ==========
async function createOrUpdateUserDocument(user) {
    ...
    try {
        ...
        // Don't throw - allow user to continue even if Firestore fails
    }
}
...
// Check for redirect result on page load - FIXED VERSION
auth.getRedirectResult().then(async result => {
    if (result.user) {
        console.log('🔐 Sign-in successful via redirect:', result.user.email || result.user.phoneNumber || result.user.uid);
        
        // **FIX** - Create/update user document in Firestore
        await createOrUpdateUserDocument(result.user);
        
        closeAuthModal();
        if (window.closePhoneModal) closePhoneModal();
        
        // Show success notification
        showNotification('Successfully signed in!', 'success');
    } else if (result.credential) {
        console.log('Got credential from redirect');
    }
}).catch(error => {
    ...
    if (error.code && error.code !== 'auth/null-user') {
        ...
        switch (error.code) {
            case 'auth/unauthorized-domain':
                errorMsg = '⚠️ DOMAIN NOT AUTHORIZED\n\nTo fix this:\n1. Go to Firebase Console (console.firebase.google.com)\n2. Select your project: webbook-9d8ec\n3. Go to Authentication → Settings → Authorized domains\n4. Click "Add domain"\n5. Add: ' + window.location.hostname + '\n6. Save and try logging in again';
                setTimeout(() => alert(errorMsg), 500);
                break;
            case 'auth/account-exists-with-different-credential':
                errorMsg = 'An account with this email already exists using a different sign-in method. Please use your original sign-in method.';
                setTimeout(() => showNotification(errorMsg, 'error'), 500);
                break;
...
// Also update onAuthStateChanged to create user doc for new users
auth.onAuthStateChanged(async user => {
    ...
    if (user) {
        ...
        // Create/update user document when auth state changes (backup mechanism)
...
function updateAuthUI() {
    ...
    if (currentUser) {
        ...
        // Handle different user types (email, phone, social)
        ...
        console.log('✅ UI updated - showing logged in state');
    } else {
        // User is logged out
        ...
        console.log('ℹ️ UI updated - showing logged out state');
    }
}

...
auth.onAuthStateChanged(async (user) => {
    ...
    if (window.adminManager) {
        window.adminManager.init(ADMIN_EMAILS, user);
    }
    ...
    if (user) {
        ...
        if (currentChapter && window.commentSystem) {
            const chapterId = `${currentSeries.id}_${currentBook.id}_${currentChapter.id}`;
            await window.commentSystem.init(chapterId, user, ADMIN_EMAILS);
        }
        ...
        // Initialize comment system for guest users
        if (currentChapter && window.commentSystem) {
            const chapterId = `${currentSeries.id}_${currentBook.id}_${currentChapter.id}`;
            await window.commentSystem.init(chapterId, null, ADMIN_EMAILS);
        }
...
async function loadChapterPage(seriesId, bookId, chapterId) {
    ...
    if (window.commentSystem) {
        const chapterId = `${seriesId}_${bookId}_${chapterId}`;
        await window.commentSystem.init(chapterId, currentUser, ADMIN_EMAILS);
    }

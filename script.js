...
document.getElementById('google-login').addEventListener('click', async () => {
    ...
    try {
        ...
        if (errorDiv) {
            errorDiv.textContent = 'Google sign in failed. Please try again or use email/password.';
        }
    }
});
    ...
    seriesData.forEach((series) => {
        ...
        seriesCard.innerHTML = `
            ...
            <div class="series-details">
                ...
                <p class="series-stats">${series.books.length} books • ${totalChapters} chapters</p>
            </div>
        `;
        seriesCard.addEventListener('click', () => loadSeriesView(series.id));
        seriesList.appendChild(seriesCard);
    });
}

function loadSeriesView(seriesId) {
    currentSeries = seriesData.find(s => s.id === seriesId);
    if (!currentSeries) return;

    // Update series header
    document.getElementById('series-title').textContent = currentSeries.name;
    document.getElementById('series-description').textContent = currentSeries.description;
    document.getElementById('series-cover').src = currentSeries.cover;
    
    const totalChapters = currentSeries.books.reduce((sum, book) => sum + book.chapters.length, 0);
    document.getElementById('series-meta').textContent = `${currentSeries.books.length} books • ${totalChapters} total chapters`;
    ...
    currentSeries.books.forEach((book) => {
        ...
        bookItem.innerHTML = `
            ...
            <p class="book-chapters-count">${book.chapters.length} chapters</p>
        `;
        bookItem.addEventListener('click', () => loadBook(seriesId, book.id));
...
async function loadBook(seriesId, bookId) {
    currentSeries = seriesData.find(s => s.id === seriesId);
    ...
    for (const chapter of currentBook.chapters) {
        ...
        `;
        chapterItem.addEventListener('click', () => loadChapter(seriesId, bookId, chapter.id));
        chaptersList.appendChild(chapterItem);
    }

    // Set back button
    document.getElementById('back-to-series').onclick = () => loadSeriesView(seriesId);
...
async function loadChapter(seriesId, bookId, chapterId) {
    ...
    if (currentIndex > 0) {
        ...
        prevBtn.onclick = () => loadChapter(seriesId, bookId, currentBook.chapters[currentIndex - 1].id);
    } else {
    ...
    if (currentIndex < currentBook.chapters.length - 1) {
        ...
        nextBtn.onclick = () => loadChapter(seriesId, bookId, currentBook.chapters[currentIndex + 1].id);
    } else {
        nextBtn.disabled = true;
    }

    // Set back button
    document.getElementById('back-to-book').onclick = () => loadBook(seriesId, bookId);
...
async function incrementViews(seriesId, bookId, chapterId) {
    const docRef = db.collection('chapters').doc(`series${seriesId}_book${bookId}_chapter${chapterId}`);

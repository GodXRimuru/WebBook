...
// These emails work for ALL login methods (Google, GitHub, Facebook, Twitter, Email/Password)
const ADMIN_EMAILS = [
    'gamersandip872@gmail.com',  // Your email (case-insensitive)
...
auth.onAuthStateChanged(async (user) => {
    ...
    if (user) {
        ...
        // Initialize user profile and check admin status
        if (window.initializeUserProfile) {
            await window.initializeUserProfile(user);
        }
        ...
        if (currentSeries && currentBook && currentChapter) {
            if (window.commentSystemFixed) {
                await window.commentSystemFixed.initialize(currentSeries.id, currentBook.id, currentChapter.id);
            } else if (window.commentSystem) {
                await window.commentSystem.initialize(currentSeries.id, currentBook.id, currentChapter.id);
            }
        }
    } else {
...
function showPage(pageId) {
    ...
    if (pageId !== 'chapter-page') {
        if (window.commentSystemFixed) {
            window.commentSystemFixed.cleanup();
        } else if (window.commentSystem) {
...
async function loadChapterPage(seriesId, bookId, chapterId) {
    ...
    if (window.commentSystemFixed) {
        await window.commentSystemFixed.initialize(seriesId, bookId, chapterId);
        if (window.setupFixedCommentForm) {
            window.setupFixedCommentForm();
        }
    } else if (window.commentSystem) {
        await window.commentSystem.initialize(seriesId, bookId, chapterId);
        if (window.setupEnhancedCommentForm) {
...
function updateReactionButtons(reaction) {
    // Use animated version if available, otherwise use basic version
    if (typeof updateReactionButtonsAnimated !== 'undefined') {
        updateReactionButtonsAnimated(reaction);
    } else {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Configure admin emails for comment system
    if (window.commentSystemFixed) {
        window.commentSystemFixed.setAdminEmails(ADMIN_EMAILS);
    }
});
